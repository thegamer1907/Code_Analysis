...
int main()
{
    ...
    static string s[500016];
    int n;

    cin >> n;
    for (int i = 0; i < n; ++i) cin >> s[i];
    for (int i = n-2; i >= 0; --i) {
        string &cur = s[i], &nxt = s[i+1];
        int lcp = 0;
        while (lcp < (int)min(cur.size(), nxt.size()) && cur[lcp] == nxt[lcp]) ++lcp;
        ctif(lcp == (int)cur.size());
        ctif(cur[lcp] < nxt[lcp]);
        cur.erase(cur.be()+lcp, cur.end());
    }
