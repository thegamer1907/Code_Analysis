...
int calc(int from,int to,int behind,int front){
	...
	int &ref = dp[from][to][behind][front],id = 0,x;
	if(ref != -1)return ref;
	for(auto i : g[to]) 
		if(i != from)
			for(int j = 1;j <= front;j++)
				calc(to,i,behind + front - j,j);
	for(auto i : g[to]){
		id++;
		if(i != from){
			for(int j = 1;j <= front;j++)temp[j] = calc(to,i,behind + front - j,j);
			break;
		}
	}
	while(id < g[to].size()){
		...
		for(int i = front;i;i--)
			for(int j = 1;i - j >= 0;j++)
				temp[i] = max(temp[i],min(temp[i - j],calc(to,x,behind + front - j,j)));
	}
