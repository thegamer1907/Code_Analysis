...
inline int solve(const int cur, const int le, const int ri)
{
    ...
    if (strs[le][0].sz >= MAX_ANSWER << 1) {
        strs[cur][0] = strs[le][0];
    } else {
        strs[cur][0] = strs[le][0] + strs[ri][0];
    }

    if (strs[ri][1].sz >= MAX_ANSWER << 1) {
...
int main()
{
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int k = 0; k < lx && k < MAX_ANSWER << 1; k++) {
            t += s[i][k], u += s[i][max(0, lx-(MAX_ANSWER << 1))+k];
        }

        const string x = s[i];
        for (int j = 1; j <= MAX_ANSWER && j <= lx; j++) {
