...
template<class T> struct RadixSort {
    ...
    static const int H = (sizeof(int) * 4 + W - 1) / W;
    static const int MSK = (1 << H) - 1;
    T tmp[MAXN];
    int cnt[1 << H];
    int p[1 << H];
    
    void sort(vector<T>& st) {
        int n = sz(st);
        for (int step = 0; step < W; step++) {
            for (int i = 0; i < n; i++) {
                cnt[st[i].fi.fi >> step * H & MSK]++;
                tmp[i] = st[i];
            }
            p[0] = 0;
            for (int i = 1; i < (1 << H); i++) {
            ...
            cnt[(1 << H) - 1] = 0;
            for (int i = 0; i < n; i++) {
                st[p[tmp[i].fi.fi >> step * H & MSK]++] = tmp[i];
            }
        }
    }
    
};
RadixSort<pair<pi, int> > radix;
...
int calc(int u, int p, int t1, int t2) {
    ...
    FOR(i, 0, sz(adj[u])) {
        ...
        if (v != p) {
            FOR(tt, 1, t1 + 1) {
                vals.pb(mp(mp(calc(v, u, tt, t2 + t1 - tt) + w, tt), v));
            }
        }
    }
    if (!sz(vals)) {
    ...
    radix.sort(vals), reverse(all(vals));
    int totmn = 0, totmx = 0;
    FOR(i, 0, sz(vals)) {
        int v = vals[i].se;
        int tt = vals[i].fi.se;
