...
void compute(int k,int l,int r,int optl,int optr){
	...
	int p = m;
	for(int i=min(m,optr);i>=max(optl,1);i--){
		maintain(i,m);
		if(i<=optr){
	...
	if(l<r){
		q.push(iiii(ii(m+1,r),ii(p,optr)));
		q.push(iiii(ii(l,m),ii(optl,p)));
...
int main(){
	...
	for(int j=0;j<k;j++){
		if(j==0){
			...
			q.push(iiii(ii(0,n-1),ii(0,n-1)));
			while(!q.empty()){
				iiii u = q.front();
				q.pop();
				compute(j,u.first.first,u.first.second,u.second.first,u.second.second);
			}
