...
pair<int,ll> get(int mid, int ck, int optL, int optR){
    //cout << "!!"<<curl << " " << curr << " " << optL << " " << optR << " " << curcost<<endl;
    ...
    //cout << "BETWEEN " << curl<<" AND " << curr << " IS " << curcost<<endl;
    ...
    for(int pp = optR; pp >=optL; pp--){
        ...
        //minival=min(minival,curcost+((pp==0)?0:dp[pp-1][ck-1]));
