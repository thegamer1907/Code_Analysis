...
void go(int i)
{
    ...
    node[i].sfix=node[i].pfix.substr(max(0,(int)node[i].pfix.size()-500),500);
    node[i].pfix=node[i].pfix.substr(0,500);
}
...
int main()
{
    ...
    for(i=n+1;i<=n+m;i++){
        ...
        go(i);
        string str=node[a].pfix+node[b].sfix;
        node[i].pfix=str.substr(0,500);
        node[i].sfix=str.substr(max(0,(int)str.size()-500),500);
