...
int main() {
    ...
    fr(i, n){
        ll x;
        cin >> x;
        mp[x / n].emplace_back(x % n, i);
    }
    fr(i, mp.begin() -> second.size()){
        if(mp.begin() -> second[i].first - mp.begin() -> second[i].second <= 0){
            cout << mp.begin() -> second[i].second + 1;
            ret
        }
    }
    if(mp.size() == 1){
    ...
    auto kek = mp.begin();
    ++kek;
    if(kek -> first - mp.begin() -> first > 1){
        cout << mp.begin() -> second[0].second + 1;
        ret
    }
    fr(i, kek -> second.size()){
        if(kek -> second[i].first - kek -> second[i].second <= 0){
            cout << min(kek -> second[i].second + 1, mp.begin() -> second[0].second + 1);
            ret
        }
    }
    cout << mp.begin() -> second[0].second + 1;
