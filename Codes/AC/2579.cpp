...
vector<bool> primes(1e7+1001, 1); //primes[i] is true if i is prime
vector<int> plist;
void prime() {
	primes[0] = 0;
	primes[1] = 0;
	for (int i = 2; i <= sqrt(1e7+1000); i++) {
		if (primes[i]) {
			for (int j = i*i; j <= 1e7+1000; j += i) {
				primes[j] = 0;
			}
		}
	}
	for (int i = 2; i <= 1e7+1000; i++) {
		if (primes[i])
			plist.push_back(i);
	}
}
...
/*	for (int i : x) {
	...
	vector<ll> cumsum(plist.size()+1);
	cumsum[0] = 0;
	for (int i = 1; i <= plist.size(); i++) {
		cumsum[i] = cumsum[i-1] + f[i-1];
	}
