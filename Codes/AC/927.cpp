...
LL Fun( LL left , LL right , LL id )
{
    LL mid , res = 0 , pos = left ;
    while( left <= right )
    {
        mid = left + ( right - left )  / 2 ;
        if( a[mid].b[id] - a[pos-1].b[id] <= m )
        {
            res = mid - pos + 1 ;
            left = mid + 1 ;
        }
...
{
    return max( Fun( left , right , 0 ) , Fun( left , right , 1 ) ) ;
}
...
int main()
{
    ...
    F( i , 0 , n - 1 )
    {
        if( s[i] == 'a' ) a[i+1].b[0] = 1 ;
        else a[i+1].b[1] = 1 ;
    }
    F( i , 1 , n ){ a[i].b[0] += a[i-1].b[0] ; a[i].b[1] += a[i-1].b[1] ; }
    LL res = 0 ;
    F( i , 1 , n )
    {
