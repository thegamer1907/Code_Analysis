#include <iostream>
#include <vector>

using namespace std;

#define ll long long
#define MAXN 1000001

int NH = 2;
vector <int> alpha = {37, 53}, MOD = {1000000007, 1000000009};
vector <vector<int>> powAlpha;

int mult(int a, int b, int t)
{
    ll res = ((ll) a * b) % MOD[t];
    return res;
}

void precompute()
{
    powAlpha.resize(NH);
    for (int t = 0; t < NH; t++)
    {
        powAlpha[t].resize(MAXN);
        powAlpha[t][0] = 1;
        for (int i = 1; i < MAXN; i++)
        {
            powAlpha[t][i] = mult(alpha[t], powAlpha[t][i - 1], t);
        }
    }
}

struct subHash
{
    int hashvalue, t, power;

    bool operator == (subHash o) const
    {
        int hv1 = hashvalue, hv2 = o.hashvalue, diff = o.power - power;
        if (diff > 0) return mult(hv1, powAlpha[t][diff], t) == hv2;
        else mult(hv2, powAlpha[t][-diff], t) == hv1;
    }
};

struct hasher
{
    int len;
    vector <vector<int>> prefixHash;

    hasher() { prefixHash.resize(NH); }

    void add(char c)
    {
        for (int t = 0; t < NH; t++)
            prefixHash[t].push_back((prefixHash[t][len] + mult(powAlpha[t][len + 1], (c - 'a' + 1), t)) % MOD[t]);
        len++;
    }

    void initialize(string &s)
    {
        len = 0;
        for (int t = 0; t < NH; t++) prefixHash[t] = {0};
        for (int i = 0; i < s.size(); i++) add(s[i]);
    }

    vector <subHash> query(int l, int r)
    {
        vector <subHash> res(NH);
        for (int t = 0; t < NH; t++)
        {
            res[t] = {(prefixHash[t][r] - prefixHash[t][l - 1] + MOD[t]) % MOD[t], t, l};
        }
        return res;
    }
};

hasher textHash;
string text;

bool test(int wordLen)
{
    auto subtext = textHash.query(1, wordLen);
    for (int i = 2; i + wordLen - 1 <= text.length() - 1; i++)
    {
        auto subword = textHash.query(i, i + wordLen - 1);
        if (subtext == subword)
        {
            return true;
        }
    }
    return false;
}

int main()
{
    ios_base::sync_with_stdio(false);
    cin.tie(NULL);

    precompute();
    cin>>text;
    textHash.initialize(text);

    vector <int> candidates;

    for (int i = 1; i <= text.length() - 1; i++)
    {
        auto prefix = textHash.query(1, i);
        auto suffix = textHash.query(text.length() - i + 1, text.length());

        if (prefix == suffix)
        {
            candidates.push_back(i);
        }
    }

    if (candidates.size() > 0)
    {
        int l = 0;
        int r = candidates.size() - 1;

        while (l < r)
        {
            int m = (l + r + 1) / 2;
            bool res = test(candidates[m]);

            if (res)
            {
                l = m;
            }
            else
            {
                r = m - 1;
            }
        }

        if (test(candidates[l]))
        {
            cout<<text.substr(0, candidates[l])<<'\n';
            return 0;
        }
    }

    cout<<"Just a legend"<<'\n';

    return 0;
}
