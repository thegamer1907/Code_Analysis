...
const int maxn = 1e6+5, base = 29, M = 1e9+7;
ll inverse[maxn];
...
struct Hash
{
    ...
    ll sub(int l, int r) {
        //hash of [l,r] inclusive
        return ((prefix[r]-prefix[l-1]+M)*inverse[l-1])%M;
    }
    bool contains(ll h, int sz) {
        //checks if hash has a substring of length sz with hash h
        //checks NOT prefix or suffix
...
int main()
{
    ...
    for (int l = h.len - 2; l >= 1; --l) {
        if (h.sub(1,l) == h.sub(h.len-l+1,h.len)) {
            if (h.contains(h.sub(1,l),l)) {
                //found the answer
                cout << s.substr(0,l) << '\n';
                return 0;
            }
