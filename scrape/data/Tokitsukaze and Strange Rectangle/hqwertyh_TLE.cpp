...
void modify (int pos, int st, int en, int rt) {
	...
	int md = (st + en) >> 1;
	if (pos <= md) modify(pos, st, md, rt << 1);
	else modify(pos, md + 1, en, rt << 1 | 1);
	c[rt] = c[rt << 1] + c[rt << 1 | 1];
}

int query (int l, int r, int st, int en, int rt) {
	if (st == en) return c[rt];
	int md = (st + en) >> 1, ret = 0;
	if (l <= md) ret += query(l, r, st, md, rt << 1);
	if (r > md) ret += query(l, r, md + 1, en, rt << 1 | 1);
	return ret;
}
