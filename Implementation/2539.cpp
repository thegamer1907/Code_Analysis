...
#endif
    readln(n , h);person.resize(n);int ans = 0;
    readln(person);
    rep(i , 0 , n - 1){
        if(person[i] > h) {
            ans += 2;
...
template<class Type> inline void print(vector<vector<Type> >& a){if(a.size())(a.size()==1)?print(a[0]):writeln2(a[0]);for(register int i=0;i<(int)a.size()-1;++i)writeln2(a[i]);if(a.size()>=2)print_no_space(a.back());}
template<class Type> inline void print_no_space(vector<Type>& a){if(a.size())print_no_space(a[0]);for(register int i=0;i<(int)a.size();++i)print(a[i]);}
