...
#define cin_fast ios_base::sync_with_stdio(false);cin.tie(NULL);cout.tie(NULL);
#define ff first
#define ss second
...
///////////                                                                        prime generator                                                 ///////////
/*************************************************************************************************/
vector<int> prigen_c_1(){                                                   //    highest prime factor
       vector<int> p(BIG);
...
vector<int> prigen_c_2(){                             // generates least prime factors
    vector<int> p(BIG);
    for(int i=2;i<BIG;i++){
	   if(i%2==0)
	       p[i]=2;
...
private:
	struct vertex{
                       ...
                       bool known;
                       vertex* path;
                       vertex(T k):key(k),in_deg(0),top_num(0),dist(inf),known(false){}
	};
	ll no_of_vertices=0;

public:
	map<T,vertex*> m;
	ll no_vertices(){ return no_of_vertices;}
    void topsort(){
	        queue<vertex*> q;
	        int counter=0;
    ...
    void unweighted(vertex *s){
		...
		q.pop();
		for(int i=0;i<(v->adj).size();i++){
			if(v->adj[i].ss->dist==inf){
			      (v->adj[i].ss)->dist=v->dist+1;
			      (v->adj[i].ss)->path=v;
			      q.push((v->adj[i].ss));
			}
		}
