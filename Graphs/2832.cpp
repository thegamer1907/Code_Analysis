...
#define TESTS(x) {DI(tests);F(test,1,tests){x}}
...
#define vlnn(v) ln(v.size());for (auto x: v) ln(x)
...
ld dist(pll x, pll y) { return sqrt(sqDist(x, y)); }
...
int main() {
	...
	FI(m) {
		...
		if (st[x].size() > st[mx].size()) {
			mx = x;
		}
		if (st[y].size() > st[mx].size()) {
			mx = y;
		}
		if (st[x].size() == st[mx].size()) {
			mx = min(x, mx);
