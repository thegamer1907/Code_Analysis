...
static void split(TreapVert *t, Vdes key, TreapVert* &l, TreapVert* &r) {
    if (!t)
        l = r = nullptr;
    else if (key < t->data)
        r = t, split(t->left, key, l, t->left);
    else
        l = t, split(t->right, key, t->right, r);
}

static void insert (TreapVert* &t, TreapVert* it) {
    if (!t)
        t = it;
    else if (it->prior > t->prior) {
        auto tmp = t;
        t = it, split(tmp, it->data, it->left, it->right);
    } else 
        insert(it->data < t->data ? t->left : t->right, it);
}

static void merge (TreapVert* & t, TreapVert* l, TreapVert* r) {
    if (!l || !r) {
        t = l ? l : r;
...
static void erase (TreapVert* & t, Vdes key) {
    if (t->data == key) {
        freeVert(t);
